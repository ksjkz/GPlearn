...
class ExpressionTree:
    def __init__(self, input): 
        '''
        如果输入是文字公式,形如mul(low, close)',则用ast解析生成node
        如果输入是node，直接赋值给root
        '''
        if isinstance(input, str):
           self.root = self._parse_expression(input)
        elif isinstance(input, Node):
            self.root = input.deepcopy()
        else :
            self.root = None
            warnings.warn("Input is not a string or a Node")
    @property
    def root(self):
        return self._root
    ...
    def root(self, new_root):
        if not isinstance(new_root, Node) and new_root is not None:
            raise TypeError("root must be a Node or None")
        self._root = new_root.deepcopy()
    ...
    def __str__(self) -> str:
        return self.get_reconstruct_expression()
    ...
    def get_depth(self,):
        ...
        返回树的深度
        '''
        return get_depth_of_node(self.root)

    
    def get_time_series_lenth(self,columns_list,print_opt=False,):
        '''
        获取时间序列长度
        '''
        return compute_time_series_length(self.root,print_opt=print_opt,columnss_list=columns_list)
...
def get_depth_of_node(node:Node):
    if not node or not node.children:  # 如果没有子节点，深度为 1（根节点本身）
        return 1
    else:
        # 递归计算每个子树的深度，并取最大值，加上当前节点
        return 1 + max(get_depth_of_node(child) for child in node.children)
...
def compute_time_series_length(node:Node, cumulative_length=0,print_opt=False, columnss_list:list=columns_list):
    """
    输入 node
    返回值 公式时序长度-1
    即dfgroupby后前多少列为naN
...
def f_coding(input:dict|str|Node,df:pd.DataFrame,drop_opt=True,std_opt=False,zero_opt=False,relu_opt=False,time_name:str='',ticker_name:str='',columns_list:list=[])-> str|bool:
    '''
    输入dict形如:
    ...
    输入str,node 即公式本身,类名为16位随机字符串 建议输入上面格式的dict
    ...
    match input:
        ...
        case str():
            print("输入是一个字符串 (str)")
            input1 = input
            name_new = generate_random_string(16)
        ...
        case _:
            print('----------输入类型错误,必须是str或者dict或者node,dict的key为"Factor Name""Factor Formula"--------')
    ...
    expr_tree = ExpressionTree(input=input1,)
    if expr_tree==False:
    ...
    if ticker_name!='':
        mm=expr_tree.get_time_series_lenth(columns_list=columns_list)
...
def f_coding_printfree(input:dict|str|Node,df:pd.DataFrame,drop_opt=True,std_opt=False,zero_opt=False,relu_opt=False,time_name:str='',ticker_name:str='',columns_list:list=[])-> str|bool:
    '''
    输入dict形如:
    "Factor Name": "AA_2_price_volatility_and_trading_activity",
    "Factor Formula": "div(ts_std(sub(ADJ_D_HIGH, ADJ_D_LOW), 10), ts_timeweighted_mean(ADJ_D_CLOSE, 10)) * sqrt(YUAN_VOLUME) + div(ts_max(ADJ_D_HIGH, 10), ts_min(ADJ_D_LOW, 10)) * log(YUAN_VOLUME)"
